"""
EXPERIMENT 2 -- Model A: dense classical head (the big classical reference).

Frozen Beast -> Linear(128,38) -> ReLU -> Dropout -> Linear(38,4).
~5058 trainable params. The high-capacity classical head that both 556-param
heads (quantum B and Tanh-mirror D) are shown to match. Output:
results_exp3_classical_4class_100epochs/.
"""
import sys, os
...
from src.models.exp3_models import FrozenBeastDenseClassifier

torch.manual_seed(42)
np.random.seed(42)

# ---------------------------------------------------------------------------
# Experiment 2 — same 4-class hard task as the quantum side
# ---------------------------------------------------------------------------
CLASSES = [
    "STAR_BROWN_DWARF_L",
    "STAR_M8",
...
HIDDEN_DIM = 38  # Linear(128,38) + Linear(38,4) ≈ 5058 trainable params
...
def main():
    ...
    results_dir = f"results_exp3_classical_{len(CLASSES)}class_100epochs"
    ...
    # Classical dense head with frozen Beast extractor
    model = FrozenBeastDenseClassifier(
    ...
    print(f"\nFrozenBeast + Dense head")
    print(f"  Trainable params: {trainable:,}  ← target ~5000 (MLP head only)")
    print(f"  Frozen params:    {frozen:,}  (Beast extractor)")

    trainer = SDSSPerformanceTrainer(model, training_config, run_name="Exp3Classical")
    ...
    list_of_runs = glob.glob("runs/Exp3Classical_*")
    if list_of_runs:
    ...
    print(f"\n[Exp3 Classical] Trainable params: {trainable:,}")
