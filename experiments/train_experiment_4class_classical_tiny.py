"""
EXPERIMENT 2 -- Model C: param-matched classical control, ReLU (the broken one).

Frozen Beast -> Linear(128,4) -> Tanh -> Linear(4,4) -> ReLU -> Linear(4,4).
556 trainable params, identical pipeline shape to quantum head B but VQC replaced
by a classical block. The inner ReLU dies on the width-4 layer -> caps ~79% and
collapses STAR_BROWN_DWARF_L (0.18 recall). The Tanh control D proves this is an
artifact, not a capacity limit. Output: results_exp3b_tiny_classical_4class/.
"""
import sys, os
...
from src.models.exp3_models import FrozenBeastTinyClassicalClassifier

torch.manual_seed(42)
np.random.seed(42)

# ---------------------------------------------------------------------------
# Experiment 2 — Tiny Classical Baseline (Parameter-Matched to VQC)
# ---------------------------------------------------------------------------
CLASSES = [
    "STAR_BROWN_DWARF_L",
    "STAR_M8",
...
def main():
    ...
    # Keep results separated from the quantum run
    results_dir = f"results_exp3b_tiny_classical_{len(CLASSES)}class"
    ...
    # Instantiate the parameter-matched classical model
    model = FrozenBeastTinyClassicalClassifier(
    ...
    print(f"\nFrozenBeast + Tiny Classical Head")
    print(f"  Trainable params: {trainable}  ← Exactly matches the Hybrid model!")
    print(f"  Frozen params:    {frozen:,}  (Beast extractor)")

    # Name the run uniquely so it doesn't overwrite your quantum logs
    trainer = SDSSPerformanceTrainer(model, training_config, run_name="Exp3bTinyClassical")
    ...
    list_of_runs = glob.glob("runs/Exp3TinyClassical_*")
    if list_of_runs:
    ...
    print(f"\n[Exp3 Tiny Classical] Trainable params: {trainable}")
